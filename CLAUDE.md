...
- [ ] **대표 강의 선택 UX 개선**: 어드민 화면에서 ID 수동 입력 대신 강의 목록을 조회하고 선택해서 대표 강의로 지정
- [ ] **강의 카드 레이아웃 안정화 검토**: 브라우저 크기 변경 시 카드 간격/폭이 흔들리지 않도록 그리드 전략 점검 (`auto-fit`, `minmax`, breakpoint`, 고정 column 수 등)
